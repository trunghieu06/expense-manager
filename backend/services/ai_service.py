...
"""
    ...
    @staticmethod
    def analyze_insights(context: str, data: list[dict]) -> str:
        api_key = os.environ.get("GROQ_API_KEY")
        if not api_key or api_key == "YOUR_OPENAI_API_KEY_HERE" or not OpenAI:
            return "Vui lòng cấu hình API Key trong file .env để AI có thể phân tích nhé!"
            
        if not data:
            return "Chưa có dữ liệu chi tiêu nào để phân tích cả."
...
Hãy phân tích dữ liệu trên và đưa ra lời khuyên hoặc nhận xét (khoảng 3-4 câu).
QUAN TRỌNG: Hãy sử dụng giọng điệu HÀI HƯỚC, CHÂM BIẾM, "XÉO XẮT" một chút để cảnh tỉnh nếu tôi tiêu pha bừa bãi hoặc mua sắm/trà sữa quá nhiều. Đừng nói chuyện như một cái máy khô khan, hãy như một người bạn thân đang "mắng yêu" vì tôi nghèo mà hay ra dẻ.
Chỉ trả về nội dung đoạn văn, không thêm các câu mào đầu.
"""
            completion = client.chat.completions.create(
                model="llama-3.1-8b-instant",
                messages=[
                    {"role": "system", "content": "Bạn là chuyên gia tài chính cá nhân siêu 'xéo xắt' và hài hước, luôn thích châm biếm thói quen tiêu xài hoang phí của người dùng."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=300
            )
            ...
            print(f"Lỗi khi phân tích AI: {e}")
            return "AI đang đi uống trà sữa rồi, không phân tích được đâu. Thử lại sau nhé!"
